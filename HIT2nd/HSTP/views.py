...
def tracking(company, number):
    req_link = "http://www.kuaidi100.com/query?type=%s&postid=%s"%(str(company), str(number))
    req = Request(req_link)
    req.add_header("Accept", "application/json, text/javascript, */*; q=0.01")
    req.add_header("Host", "www.kuaidi100.com")
    req.add_header("Connection", "keep-alive")
    req.add_header("Cookie", "WWWID=WWW7DC959D8B3FAA7C86C02C69B14E1A254")
    req.add_header("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.181 Safari/537.36")
    resp = urlopen(req)
    data = eval(resp.read())
    return [data["nu"], data["data"]]

# if __name__ == "__main__":
#     print(tracking("yunda", "3914733622857"))
